...
class SalaController {
    ...
    init() {
        ...
        const inputBusca = document.querySelector("input[placeholder='Digite o nome da sala']");

        if (btnBuscar && inputBusca) {
            btnBuscar.addEventListener("click", () => this.buscarSalas(inputBusca.value));
    ...
    buscarSalas(termo) {
        ...
        const resultados = this.listaSalas.filter(sala =>
    ...
    salvarNoLocalStorage() {
        localStorage.setItem(this.LOCAL_STORAGE_KEY, JSON.stringify(this.listaSalas));
    }
    ...
    atualizarTabela(salas = this.listaSalas) {
        const tbody = document.getElementById("tabelaSalas") || document.querySelector("tbody");
        if (!tbody) return;
    ...
    abrirModalCadastro() {
        ...
        document.getElementById("idModalSalaTitulo").textContent = "Cadastrar Nova Sala";
    ...
    abrirModalExcluir(id) {
        ...
        if (sala) {
            const msgConfirmacao = document.querySelector("#modalExcluirSala .modal-body h5");
            if (msgConfirmacao) {
