...
class SessaoController {
    ...
    init() {
        ...
        if (btnExcluirSessao) btnExcluirSessao.addEventListener("click", () => this.excluir(this.idParaExcluir));
    ...
    carregarDados() {
        // Carregar filmes
        const filmesSalvos = localStorage.getItem("cinema_filmes");
        if (filmesSalvos) {
            try {
    ...
    buscarSessoes(termo) {
        ...
        const resultados = this.listaSessoes.filter(sessao => {
            ...
            return (filme && filme.titulo.toLowerCase().includes(termoLower)) ||
    ...
    salvarNoLocalStorage() {
        localStorage.setItem(this.LOCAL_STORAGE_KEY, JSON.stringify(this.listaSessoes));
    }

    atualizarTabela(sessoes = this.listaSessoes) {
        const tbody = document.getElementById("tabelaSessoes") || document.querySelector("tbody");
        if (!tbody) return;
